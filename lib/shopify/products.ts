...
/* ------------------------------------------------------------------ */
/* SKU collision avoidance                                             */
/*                                                                     */
/* Our Supabase products table doesn't yet contain every SKU that      */
/* exists in Shopify (5,000+ products predate the new ingestion        */
/* pipeline). The DB-side generator only sees rows we've inserted, so  */
/* it happily hands out NCE numbers that already exist in Shopify.     */
/* These helpers let the API layer probe Shopify before committing.    */
/* ------------------------------------------------------------------ */

export async function skuExistsInShopify(sku: string): Promise<boolean> {
  const query = `
  ...
  return (data.products.edges?.length ?? 0) > 0
}

// Returns the highest integer N such that some Shopify product has SKU "NCE{N}".
// Uses the GraphQL search to scope to our SKU prefix, then paginates. Capped at
// `maxPages` to avoid runaway calls — 50 pages * 250 results = 12,500 SKUs is
// well above current catalogue size.
export async function findMaxShopifySkuNumber(maxPages = 50): Promise<number> {
  const query = `
    query skuPage($q: String!, $cursor: String) {
      products(first: 250, query: $q, after: $cursor) {
        edges {
          cursor
          node { variants(first: 10) { edges { node { sku } } } }
        }
  ...
  type SkuPage = {
    products: {
      edges: { node: { variants: { edges: { node: { sku: string | null } }[] } } }[]
  ...
  for (let page = 0; page < maxPages; page++) {
    const data: SkuPage = await shopifyGraphQL<SkuPage>(query, { q: 'sku:NCE*', cursor })

    for (const edge of data.products.edges) {
      for (const v of edge.node.variants.edges) {
        const m = v.node.sku?.match(/^NCE(\d+)$/)
        if (m) {
