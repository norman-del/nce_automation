...
6. ~~**Owner-feedback #14b — B-Grade and Clearance condition options**.~~ Done 2026-05-06. Migration `20260506120000` widens `products.condition` + `warranty_templates.applies_to_condition` CHECK constraints to accept `'b-grade'` and `'clearance'`. Strategic ingestion + edit forms swap the condition `<select>` for a four-pill toggle group with distinct colours; selecting any non-`new` condition auto-defaults `vat_applicable=false`. Merchant feed maps both new conditions → Google Shopping `"used"`. Bridge code untouched; storefront-side BrandBadge rendering is a small Gus follow-up. See `docs/handoffs/session-2026-05-06.md`.
7. ~~**Owner-feedback #16(b) — vendor logo bank with auto-assign**.~~ Done 2026-05-06. Migration `20260506130000` creates `vendor_logos` table (handle PK, name, aliases text[], logo_url, storage_path, content_type) + `products.vendor_logo_url` column + Supabase Storage bucket `vendor-logos` (public-read, 2 MB limit). 21 brand catalogue mirroring `nce-site/lib/brand-logos.ts` seeded. Resolver `lib/strategic/brand-logos.ts` does case-insensitive alias lookup; `createStrategicProduct` + `updateStrategicProduct` auto-write `vendor_logo_url` on save (form override supported). Admin-only "Brand Logos" tab in Settings supports Upload / Replace / Clear / Edit per brand; uploads land in the bucket and **backfill `products.vendor_logo_url` for every product matching the brand's aliases**.
8. ~~**Owner-feedback #9 bonus — seed logo bank from live Shopify storefront**.~~ Done 2026-05-06. Read-only scrape of `https://nationwidecatering.co.uk/` homepage trusted-brands strip via `scripts/seed-vendor-logos-from-shopify.ts` (dry-run-default). 6 brand logos uploaded (`blue-seal`, `falcon`, `foster`, `hamoki`, `lincat`, `rational`) and **53 existing products backfilled**. The other 15 brands (Adexa, Blizzard, Buffalo, Burco, Combisteel, Ecofrost, Gram, Hobart, Infernus, Polar, Prodis, Tefcold, Vogue, Williams, Winterhalter) remain text-fallback until manually uploaded — see follow-up below.
...
### Manual follow-ups (no code work)
- **15 brand logos still to upload.** Catalogue seeded but logo files missing for: Adexa, Blizzard, Buffalo, Burco, Combisteel, Ecofrost, Gram, Hobart, Infernus, Polar, Prodis, Tefcold, Vogue, Williams, Winterhalter. Norman or Rich uploads via Settings → Brand Logos. Each upload auto-backfills `products.vendor_logo_url` for every product whose vendor matches that brand's aliases. SVG preferred (logo render cleanest), PNG/WebP/JPEG also accepted; 2 MB per file.
